...
welcomemsg() { \
	dialog --title "Welcome!" --msgbox "Welcome to George's Suckless Arch Auto-Ricer!\\n\\nThis script will automatically install a fully-featured Linux desktop, which I use as my main machine.\\n\\n-George" 10 60
...
usercheck() { \
	...
	dialog --colors --title "WARNING!" --yes-label "CONTINUE" --no-label "No wait..." --yesno "The user \`$name\` already exists on this system. SAAR can install for a user already existing, but it will \\Zboverwrite\\Zn any conflicting settings/dotfiles on the user account.\\n\\nSAAR will \\Zbnot\\Zn overwrite your user files, documents, videos, etc., so don't worry about that, but only click <CONTINUE> if you don't mind your settings being overwritten.\\n\\nNote also that SAAR will change $name's password to the one you just gave." 14 70
	}
...
newperms() { # Set special sudoers settings for install (or after).
	sed -i "/#SAAR/d" /etc/sudoers
	echo "$* #SAAR" >> /etc/sudoers ;}
...
maininstall() { # Installs all needed programs from main repo.
	dialog --title "SAAR Installation" --infobox "Installing \`$1\` ($n of $total). $1 $2" 5 70
	installpkg "$1"
	}
...
gitmakeinstall() {
	...
	dialog --title "SAAR Installation" --infobox "Installing \`$progname\` ($n of $total) via \`git\` and \`make\`. $(basename "$1") $2" 5 70
...
aurinstall() { \
	dialog --title "SAAR Installation" --infobox "Installing \`$1\` ($n of $total) from the AUR. $1 $2" 5 70
	echo "$aurinstalled" | grep -q "^$1$" && return 1
	sudo -u "$name" $aurhelper -S --noconfirm "$1" >/dev/null 2>&1
	}

pipinstall() { \
	dialog --title "SAAR Installation" --infobox "Installing the Python package \`$1\` ($n of $total). $1 $2" 5 70
...
for x in curl base-devel git ntp zsh; do
	dialog --title "SAAR Installation" --infobox "Installing \`$x\` which is required to install and configure other programs." 5 70
	installpkg "$x"
done

dialog --title "SAAR Installation" --infobox "Synchronizing system time to ensure successful and secure installation of software..." 4 70
...
dialog --title "SAAR Installation" --infobox "Finally, installing \`libxft-bgra\` to enable color emoji in suckless software without crashes." 5 70
...
newperms "%wheel ALL=(ALL) ALL #SAAR
